...
4. `KeyguardMediaController`的最终显示路径不会再次读取锁屏媒体开关。SystemUI刚启动时，前述`true`缓存可能在设置同步完成前参与可见性判断；后续可见性刷新会改变表现，因此问题呈现出时序性。
5. 模块会同步Carousel与Hierarchy两份缓存，在设置变化后刷新相关可见性，并在锁屏媒体的最终显示路径再次读取当前用户的真实设置。只在锁屏、锁定通知栏或息屏状态下阻止显示，不影响解锁后的QS媒体。
